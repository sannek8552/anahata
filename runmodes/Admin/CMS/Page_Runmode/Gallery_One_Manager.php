...
class Gallery_One_Manager extends Runmode 
{
	function Gallery_One_Manager()
	{
		...
		$this->base_map[3] = 'num';
		parent::Runmode();
		$GLOBALS['core.smarty']->assign('current_sub', 'Gallery Simple');
	...
	function process()
	{
		...
		{
			$gallery_holder = new Gallery_One_Holder($_REQUEST['id']);
			$gallery_holder->assign_to_smarty();
			//$sizes = Gallery_One_Holder::get_sizes($_REQUEST['id']);
	...
	function process_picture()
	{
		...
		return Admin_Header::out('CMS/Page_Runmode/gallery_one_manager_picture');
	}
}
?>