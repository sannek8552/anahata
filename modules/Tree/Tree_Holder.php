...
class Tree_Holder extends Data_Holder 
{
	...
	function get_all($filters = array(), $additional_fields = array())
	{
		//print_r($additional_fields);
		$fields = array($this->key_info['key_name'],$this->parent_field,$this->order_field,$this->title_field);
		$fields = array_merge($fields, $additional_fields);
		
		$all_data = $GLOBALS['core.tree']->get_tree(0,$fields,$this->table,$this->key_info['key_name'],$this->parent_field,$this->order_field,$filters);
		return $all_data;
	}
        
	
	function get_submenus($parent_id,$filters = array(), $add = array())
	{
		$fields = array($this->key_info['key_name'],$this->parent_field,$this->order_field,$this->title_field);
		$fields = array_merge($fields, $add);
		$all_data = $GLOBALS['core.tree']->get_tree($parent_id,$fields,$this->table,$this->key_info['key_name'],$this->parent_field,$this->order_field,$filters);
		return $all_data;
	}
}
?>