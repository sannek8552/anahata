...
/*		array (
        ...
        $cfg['ilibs'] = array ();        
        #var_dump($GLOBALS['core.auth.user']);
        $cwd = getcwd();
        $config_path = preg_replace("/js\/.*/","conf/templates.ini",$cwd);
        #print $config_path;
        $templates = parse_ini_file($config_path);
	/*
        if (isset($GLOBALS['core.auth.user']->data['user_id']))
        {
            $cfg['ilibs'][] = array (
		'value'   	=> $templates['site_url'].'cms_images,
		'text'    	=> 'My Pictures',
            );
        }
	*/
	...
	$cfg['ilibs'] = array (																// image library path with slashes; absolute to root directory - please make sure that the directories have write permissions	
		array (																		
			'value'   	=> $templates['script_url'].'upload/',
			'text'    	=> 'Site Pictures',
		),
		array (
			'value'   	=> $templates['script_url'].'upload/thumb/',
			'text'    	=> 'Gallery',
		),
	);
